...
from ui import search_bar, result_summary, paper_network_and_basic_info, chat_panel
...
def main():
    ...
    with st.container():
        ...
        with network_col:
            ...
            if st.session_state["papers"].paper and st.button("解析開始",key="init_2"):
                
                pass

            if st.session_state["papers"].paper:
                selected, element_dict, papers_dict = paper_network_and_basic_info.render_network_sections(st.session_state["papers"])
                ...
                if selected and "nodes" in selected and selected["nodes"]:
                    if selected["nodes"] != st.session_state["prev_selected_nodes"]:
                        ...
                        st.session_state["selected_paper"] = paper_network_and_basic_info.get_selected_papers(selected, element_dict, papers_dict)
